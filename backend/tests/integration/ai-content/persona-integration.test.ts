...
describe('AI Content Generation with Persona Integration', () => {
  ...
  beforeAll(async () => {
    ...
    professionalPersona = await DatabaseHelpers.createTestPersona(
      testUser.id,
      professionalPersonaData
    );
  ...
  describe('Personalized Summary Generation', () => {
    test('should generate beginner-friendly summaries for student persona', async () => {
      ...
      try {
        ...
        const aiContent = await DatabaseHelpers.createTestAIContent(testFile.id, 'summary', {
        ...
        expect(aiContent.content.text.toLowerCase()).toMatch(
          /(basically|simply|easy|think of it)/i
        );
  ...
  describe('Adaptive Flashcard Generation', () => {
    test('should generate appropriate flashcards for different personas', async () => {
      ...
      const professionalFlashcards =
        AITestHelpers.createMockAIResponse(professionalPersona).flashcards;
      ...
      const studentText = studentCards
        .map((c: any) => c.front + ' ' + c.back)
        .join(' ')
        .toLowerCase();
      ...
      const professionalText = professionalCards
        .map((c: any) => c.front + ' ' + c.back)
        .join(' ')
        .toLowerCase();
  ...
  describe('Content Quality Assessment', () => {
    test('should evaluate content quality with persona metrics', async () => {
      ...
      const studentPersonalizedContent =
        'Hey! Machine learning is basically how computers can learn stuff from data automatically. Think of it like teaching a computer to recognize cats by showing it lots of cat pictures!';
      ...
      const professionalPersonalizedContent =
        'This comprehensive analysis examines machine learning paradigms, encompassing supervised methodologies with labeled datasets for predictive modeling applications.';
    ...
    test('should maintain quality consistency across persona adaptations', async () => {
      const baseContent =
        'Machine learning algorithms process data to make predictions and identify patterns.';
      ...
      for (const persona of personas) {
        ...
        const quality = AITestHelpers.evaluateContentQuality(
          personalizedContent,
          keywords,
          persona
        );
      ...
      personalizedScores.forEach((score) => {
        expect(score.quality.personalization_score).toBeGreaterThan(
          nonePersona.quality.personalization_score
        );
      });
    });
  });
  ...
  describe('Learning Style Adaptation', () => {
    test('should adapt content for visual learners', async () => {
      ...
      const aiContent = await DatabaseHelpers.createTestAIContent(testFile.id, 'explanation', {
      ...
      expect(aiContent.content.text.toLowerCase()).toMatch(
        /(imagine|picture|visualize|see|diagram)/i
      );
  ...
  describe('Technical Level Adaptation', () => {
    test('should generate content appropriate for different technical levels', async () => {
      ...
      expect(contentResults[0].content.complexity_score).toBeLessThan(
        contentResults[1].content.complexity_score
      );
      expect(contentResults[1].content.complexity_score).toBeLessThan(
        contentResults[2].content.complexity_score
      );
  ...
  describe('Interest-Based Personalization', () => {
    ...
    test('should adapt examples based on career focus', async () => {
      ...
      expect(aiContent.content.text.toLowerCase()).toMatch(
        /(data science|predictive|modeling|analysis)/i
      );
