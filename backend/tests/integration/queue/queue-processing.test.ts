...
import { TestDatabase, TestRedis, TestAPI, waitForCondition, PerformanceTracker } from '../../utils/test-helpers';
...
describe('Queue Processing Integration Tests', () => {
  ...
  describe('PGMQ Queue Operations', () => {
    test('should enqueue and process file processing job', async () => {
      ...
      const enqueueResponse = await fetch(
        `${testConfig.api.baseUrl}/api/v1/queue/enqueue`,
        {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${authToken}`,
      ...
      const fileStatusResponse = await fetch(
        `${testConfig.api.baseUrl}/api/v1/files/${fileId}`,
        {
          headers: { 'Authorization': `Bearer ${authToken}` },
        }
      );
    ...
    test('should handle job priorities correctly', async () => {
      ...
      // Get queue status
      const statusResponse = await fetch(
        `${testConfig.api.baseUrl}/api/v1/queue/status`,
        {
          headers: { 'Authorization': `Bearer ${authToken}` },
        }
      );
    ...
    test('should handle dead letter queue', async () => {
      ...
      await waitForCondition(async () => {
        ...
        return dlqJobs.some((job: any) => job.id === job_id);
      }, 10000);
      ...
      const dlqJobs = await dlqResponse.json();
      const failedJob = dlqJobs.find((job: any) => job.id === job_id);
  ...
  describe('Queue-Worker Coordination', () => {
    test('should coordinate between Node and Python workers', async () => {
      ...
      await waitForCondition(async () => {
        ...
        return stages.includes('text_extraction') && 
  ...
  describe('Batch Queue Processing', () => {
    test('should process multiple files in batch efficiently', async () => {
      ...
      const batchResponse = await fetch(
        `${testConfig.api.baseUrl}/api/v1/queue/batch/enqueue`,
        {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${authToken}`,
  ...
  describe('Queue Monitoring and Health', () => {
    test('should provide accurate queue metrics', async () => {
      ...
      const metricsResponse = await fetch(
        `${testConfig.api.baseUrl}/api/v1/queue/metrics`,
        {
          headers: { 'Authorization': `Bearer ${authToken}` },
        }
      );
    ...
    test('should detect and report stuck jobs', async () => {
      ...
      const healthResponse = await fetch(
        `${testConfig.api.baseUrl}/api/v1/queue/health`,
        {
          headers: { 'Authorization': `Bearer ${authToken}` },
        }
      );
      ...
      const stuckJobsIssue = health.issues.find((issue: any) => 
        issue.type === 'stuck_jobs'
      );
  ...
  describe('Queue Cleanup and Maintenance', () => {
    ...
    test('should vacuum queue tables for performance', async () => {
      ...
      const vacuumResponse = await fetch(
        `${testConfig.api.baseUrl}/api/v1/admin/queue/vacuum`,
        {
          method: 'POST',
          headers: { 'Authorization': `Bearer ${authToken}` },
        }
