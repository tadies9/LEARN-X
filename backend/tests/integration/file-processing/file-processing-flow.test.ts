...
import { TestDatabase, TestRedis, TestFiles, TestAPI, PerformanceTracker, waitForCondition } from '../../utils/test-helpers';
...
describe('File Processing Integration Tests', () => {
  ...
  describe('Complete File Processing Flow', () => {
    test('should process PDF file end-to-end', async () => {
      ...
      expect(report['file-upload'].duration).toBeLessThan(testConfig.performance.apiResponseThreshold * 2);
      expect(report['file-processing'].duration).toBeLessThan(testConfig.performance.fileProcessingThreshold);
    });
    ...
    test('should process Word document', async () => {
      ...
      const fileData = await fileResponse.json();
      expect(fileData.mime_type).toMatch(/application\/(vnd\.openxmlformats-officedocument\.wordprocessingml\.document|msword)/);
