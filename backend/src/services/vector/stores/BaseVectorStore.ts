...
export abstract class BaseVectorStore implements IVectorStore {
  ...
  async upsert(document: VectorDocument): Promise<void> {
    ...
    return this.monitor.monitorUpsert(() => this.doUpsert(document), 1, {
  ...
  async upsertBatch(documents: VectorDocument[]): Promise<BatchOperationResult> {
    ...
    return this.monitor.monitorUpsert(() => this.doUpsertBatch(documents), documents.length, {
  ...
  async search(
    ...
    return this.monitor.monitorSearch(() => this.doSearch(queryVector, defaultOptions), {
  ...
  async searchMulti(
    ...
    const results = await Promise.all(queryVectors.map((vector) => this.search(vector, options)));
  ...
  async delete(ids: string[]): Promise<BatchOperationResult> {
    ...
    return this.monitor.monitorDelete(() => this.doDelete(ids), ids.length, {
      idCount: ids.length,
    });
  }
  ...
  protected abstract doUpdateMetadata(id: string, metadata: Record<string, any>): Promise<void>;
  ...
  async clear(): Promise<void> {
    ...
    const confirmed = process.env.NODE_ENV === 'test' || process.env.ALLOW_VECTOR_CLEAR === 'true';
