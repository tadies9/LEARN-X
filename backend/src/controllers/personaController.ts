...
export class PersonaController {
  ...
  upsertPersona = async (req: Request, res: Response) => {
    try {
      const userId = req.user!.id;
  ...
  updateSection = async (req: Request, res: Response) => {
    try {
      ...
      const validSections = ['professional', 'interests', 'learningStyle', 'contentPreferences', 'communication'];
  ...
  exportPersona = async (req: Request, res: Response) => {
    try {
      const userId = req.user!.id;
  ...
  private validatePersonaData(data: any): string | null {
    if (!data.professional || !data.interests || !data.learningStyle || 
        !data.contentPreferences || !data.communication) {
      return 'All persona sections are required';
    }

    // Validate professional context
    if (!data.professional.role || !data.professional.industry || 
        !data.professional.technicalLevel || typeof data.professional.experienceYears !== 'number') {
      return 'Invalid professional context data';
    }

    // Validate interests
    if (!Array.isArray(data.interests.primary) || data.interests.primary.length === 0 ||
        !Array.isArray(data.interests.learningTopics) || data.interests.learningTopics.length === 0) {
  ...
  private personaToCSV(persona: any): string {
    ...
    if (persona.professional_context.careerAspirations) {
      rows.push(`Professional,Career Aspirations,"${persona.professional_context.careerAspirations}"`);
    }
    ...
    if (persona.personal_interests.secondary?.length) {
      rows.push(`Interests,Secondary Interests,"${persona.personal_interests.secondary.join(', ')}"`);
    }
    rows.push(`Interests,Learning Topics,"${persona.personal_interests.learningTopics.join(', ')}"`);
    ...
    rows.push(`Content,Repetition Preference,"${persona.content_preferences.repetitionPreference}"`);
    ...
    rows.push(`Communication,Encouragement Level,"${persona.communication_tone.encouragementLevel}"`);
