...
export function ReviewStep() {
  ...
  const renderProfessionalSummary = () => {
    ...
    return (
      <div className="space-y-2">
        <p><span className="font-medium">Role:</span> {prof.role}</p>
        <p><span className="font-medium">Industry:</span> {prof.industry}</p>
        <p><span className="font-medium">Experience:</span> {prof.experienceYears} years</p>
        <p><span className="font-medium">Technical Level:</span> {prof.technicalLevel}</p>
        {prof.careerAspirations && (
          <p><span className="font-medium">Aspirations:</span> {prof.careerAspirations}</p>
        )}
      </div>
    )
  }
  ...
  const renderLearningSummary = () => {
    ...
    return (
      <div className="space-y-2">
        <p><span className="font-medium">Primary:</span> {learning.primary}</p>
        {learning.secondary && (
          <p><span className="font-medium">Secondary:</span> {learning.secondary}</p>
        )}
        <p><span className="font-medium">Preference Strength:</span> {Math.round(learning.preferenceStrength * 100)}%</p>
      </div>
    )
  }
  ...
  const renderContentSummary = () => {
    ...
    return (
      <div className="space-y-2">
        <p><span className="font-medium">Density:</span> {content.density}</p>
        <p><span className="font-medium">Examples per concept:</span> {content.examplesPerConcept}</p>
        <p><span className="font-medium">Summary style:</span> {content.summaryStyle.replace('_', ' ')}</p>
        <p><span className="font-medium">Detail level:</span> {content.detailTolerance}</p>
        <p><span className="font-medium">Repetition:</span> {content.repetitionPreference}</p>
      </div>
    )
  }
  ...
  const renderCommunicationSummary = () => {
    ...
    return (
      <div className="space-y-2">
        <p><span className="font-medium">Style:</span> {comm.style.replace('_', ' ')}</p>
        <p><span className="font-medium">Technical comfort:</span> {Math.round(comm.technicalComfort * 100)}%</p>
        <p><span className="font-medium">Encouragement:</span> {comm.encouragementLevel}</p>
        <p><span className="font-medium">Humor:</span> {comm.humorAppropriate ? 'Yes' : 'No'}</p>
      </div>
    )
  }
      ...
      <CardContent className="space-y-4">
        ...
        <div className="bg-muted/50 rounded-lg p-4 mt-6">
          <p className="text-sm text-center">
            ...
            later in your account settings. Your learning profile will continue to adapt as we 
