...
export function useSyncedScrolling(options: SyncedScrollingOptions = {}) {
  ...
  const syncContentToPersonalized = useCallback(
    (scrollTop: number, pageHeight: number) => {
  ...
  const handlePersonalizedScroll = useCallback(
    (event: Event) => {
      const target = event.target as HTMLDivElement;
      if (target && !isScrollingSynced.current) {
  ...
  const scrollToSection = useCallback(
    (sectionId: string) => {
      ...
      const sectionElement = personalizedRef.current?.querySelector(
