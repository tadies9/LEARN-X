...
export const personaApi = {
  ...
  upsertPersona: async (personaData: Omit<Persona, 'id' | 'userId' | 'createdAt' | 'updatedAt' | 'version'>) => {
    const response = await apiClient.post<{ success: boolean; data: Persona }>('/persona', personaData)
    return response.data.data
  },

  // Update specific section
  updateSection: async (section: string, sectionData: any) => {
    const response = await apiClient.patch<{ success: boolean; data: Persona }>(`/persona/${section}`, sectionData)
    return response.data.data
  },
